...
class Character {
    ...
    makeCard(locationDiv, sceneName) {
        ...
        cardBody.append(`<button type="button" class="btn btn-danger" id="${id}-card-button-${sceneName}" onclick="game.chooseCharacter('${this.name}')">Choose ${this.name}</button>`)
    }
}
...
class StarWarsRPG {
    ...
    setScene(sceneID, sceneName) {
        ...
        var winLoseScene = $("#choose-character-scene");
    ...
    attack() {
        ...
        if (this.currentOpponent.hp <= 0) {
            ...
            // this.opponents.splice(this.opponents.indexOf(this.currentOpponent) - 1, 1)
            this.setScene($("#choose-opponent-scene"), "opponent-scene")
...
game.setScene($("#start-scene"))
$("#start-scene").on("click", function () {
    game.setScene($("#choose-character-scene"), "choose-scene")
})